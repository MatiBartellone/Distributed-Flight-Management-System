...
fn main() -> Result<(), Errors> {
    ...
    io::stdout()
        .flush()
        .map_err(|_| Errors::ReadTimeout("Error flushing stdout".to_string()))?;
    ...
    let mut stream = TcpStream::connect(node)
        .map_err(|_| Errors::ServerError("Error connecting to server".to_string()))?;
    let file = File::open("src/querys.txt")
        .map_err(|_| Errors::ReadTimeout("Error al leer las querys".to_string()))?;
    ...
    for line in reader.lines() {
        ...
        stream
            ...
            .map_err(|_| Errors::ServerError("Error writing to socket".to_string()))?;
        stream
            .flush()
