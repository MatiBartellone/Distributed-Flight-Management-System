...
impl CassandraConnection {
    ...
    fn write_stream(&mut self, frame: String) -> Result<(), String> {
    ...
    fn read_stream(&mut self) -> Result<String, String> {
        let mut buf = [0; 1024];
        match self.stream.read(&mut buf) {
            Ok(n) if n > 0 => {
                String::from_utf8(buf[..n].to_vec())
                    .map_err(|_| "Error al convertir los bytes a String".to_string())
            }
    ...
    pub fn send_and_receive(& mut self, frame: String) -> Result<String, String> {
