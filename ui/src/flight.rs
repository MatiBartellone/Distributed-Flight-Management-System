...
impl Flight {
    ...
    pub fn draw_image_flight(&self, response: &Response, painter: Painter, projector: &Projector){
        ...
        let angle = calculate_angle_to_airport(self.get_flight_pos2(projector), screen_airport_position);
        image.angle(angle);
    ...
    fn clickeable_flight(&self, response: &Response, projector: &Projector, on_flight_selected: &Arc<Mutex<Option<Flight>>>){
        let screen_flight_position = self.get_flight_pos2(projector);
        if self.is_hovering_on_flight(response, screen_flight_position) && response.clicked_by(egui::PointerButton::Primary) {
