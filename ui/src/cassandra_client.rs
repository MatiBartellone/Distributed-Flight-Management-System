...
use crate::{
    airport_implementation::airport::Airport,
    flight_implementation::{
        flight::Flight, flight_selected::FlightSelected, flight_status::FlightStatus,
    },
    utils::{
...
impl CassandraClient {
    ...
    pub fn get_airport(&mut self, airports_code: String) -> Option<Airport> {
        let query = format!(
            "SELECT name, positionLon, positionLat, code FROM aviation.airports WHERE code = '{}';",
            airports_code
        );
        let header = vec![
            "name".to_string(),
    ...
    fn row_to_flight(
        &self,
        body_strong: &[u8],
        header_stong: Vec<String>,
        body_weak: &[u8],
        header_weak: Vec<String>,
    ) -> Option<Flight> {
        let strong_row = self
            .get_rows(body_strong, header_stong)?
            .into_iter()
            .next()?;
    ...
    fn row_to_flight_selected(
        &self,
        row_strong: &[u8],
        header_strong: Vec<String>,
        row_weak: &[u8],
        header_weak: Vec<String>,
    ) -> Option<FlightSelected> {
        let strong_row = self
