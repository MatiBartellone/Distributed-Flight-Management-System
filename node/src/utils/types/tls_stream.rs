...
pub fn flush_tls_stream(stream: &mut StreamOwned<ServerConnection, TcpStream>) -> Result<(), Errors> {
    stream
        .flush()
        .map_err(|_| ServerError(String::from("Failed to flush stream")))
}

pub fn write_to_tls_stream(stream: &mut StreamOwned<ServerConnection, TcpStream>, content: &[u8]) -> Result<(), Errors> {
    stream
        .write_all(content)
        .map_err(|_| ServerError(String::from("Failed to write to stream")))
}

pub fn read_exact_from_tls_stream(stream: &mut StreamOwned<ServerConnection, TcpStream>) -> Result<Vec<u8>, Errors> {
