...
impl BytesCursor {
    ...
    pub fn read_u8(&mut self) -> Result<u8, Errors> {
        let mut buf = [0u8; 1];
        self.cursor.read_exact(&mut buf).map_err(|_| Errors::ProtocolError(String::from("Could not read byte")))?;
        Ok(buf[0])
    }

    pub fn read_i16(&mut self) -> Result<i16, Errors> {
        let mut buf = [0u8; 2];
        self.cursor.read_exact(&mut buf).map_err(|_| Errors::ProtocolError(String::from("Could not read bytes")))?;
    ...
    pub fn read_remaining_bytes(&mut self) -> Result<Vec<u8>, Errors> {
        ...
        self.cursor.read_to_end(&mut body).map_err(|_| Errors::ProtocolError(String::from("Could not read bytes")))?;
        Ok(body)
    }
    ...
    pub fn read_int(&mut self) -> Result<i32, Errors> {
        ...
        Ok(i32::from_be_bytes(buf.try_into().map_err(|_| Errors::ProtocolError(String::from("Could not read bytes")))?))
    }
    pub fn read_long(&mut self) -> Result<i64, Errors> {
        let buf = self.read_exact(8)?;
        Ok(i64::from_be_bytes(buf.try_into().map_err(|_| Errors::ProtocolError(String::from("Could not read bytes")))?))
    }
    pub fn read_short(&mut self) -> Result<i16, Errors> {
        let buf = self.read_exact(2)?;
        Ok(i16::from_be_bytes(buf.try_into().map_err(|_| Errors::ProtocolError(String::from("Could not read bytes")))?))
    }
    pub fn read_string(&mut self) -> Result<String, Errors> {
        let length = self.read_short()? as usize;
        let bytes = self.read_exact(length)?;
        String::from_utf8(bytes).map_err(|_| Errors::ProtocolError(String::from("Invalid UTF-8 string")))
    }
...
mod tests {
    ...
    fn test_read_remaining_bytes() {
        ...
        assert_eq!(cursor.read_remaining_bytes().unwrap(), vec![0x02, 0x03, 0x04]);
    ...
    #[test]
    fn test_read_string() {
        let bytes = vec![0x00, 0x0A, b'I', b'n', b'm', b'u', b't', b'a', b'b', b'l', b'e', b's', 0x02]; // length, string, extra
        let mut cursor = BytesCursor::new(&bytes);
    ...
    fn test_read_long_string() {
        let bytes = vec![0x00, 0x00, 0x00, 0x0A, b'I', b'n', b'm', b'u', b't', b'a', b'b', b'l', b'e', b's', 0x02]; // length, string, extra
        let mut cursor = BytesCursor::new(&bytes);
    ...
    fn test_read_bytes() {
        ...
        assert_eq!(cursor.read_bytes().unwrap(), Some(vec![0x01, 0x02, 0x03, 0x04]));
    }
