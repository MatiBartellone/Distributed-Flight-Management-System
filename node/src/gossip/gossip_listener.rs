...
impl GossipListener {
    ...
    fn handle_connection(stream: &mut TcpStream) -> Result<(), Errors> {
        ...
        let received_nodes : Vec<Node> = serde_json::from_slice(&buffer[..size]).expect("Failed to deserialize json");
        ...
        Self::check_differences(&cluster, received_nodes, &mut required_changes, &mut new_nodes);

        let mut stream = MetaDataHandler::establish_connection()?;
        let node_meta_data = MetaDataHandler::get_instance(&mut stream)?.get_nodes_metadata_access();
        node_meta_data.set_new_cluster(NODES_METADATA, &Cluster::new(Node::new_from_node(own_node), new_nodes))?;

        Self::send_required_changes(&mut stream, required_changes)
    }

    fn send_required_changes(stream: &mut TcpStream, required_changes: Vec<Node>) -> Result<(), Errors> {
        let serialized = serde_json::to_string(&required_changes).map_err(|_| Errors::ServerError(String::from("Failed to serialize required changes")))?;
        stream.write_all(serialized.as_bytes()).map_err(|_| Errors::ServerError(String::from("Failed to send required changes")))?;
        Ok(())
    }

    fn check_missing_nodes(cluster: &Cluster, received_nodes: &Vec<Node>, required_changes: &mut Vec<Node>)   {
    ...
    fn check_differences(cluster: &Cluster, received_nodes: Vec<Node>, required_changes: &mut Vec<Node>, new_nodes: &mut Vec<Node>) {
        let (own_node, nodes_list) = (cluster.get_own_node(), cluster.get_other_nodes());
        for received_node in received_nodes {
            if received_node.get_pos() != own_node.get_pos() {
                match Self::get_node(&received_node, nodes_list) {
                    Some(registered_node) => match Self::needs_to_update(&registered_node, &received_node) {
                        1 => {
                            required_changes.push(Node::new_from_node(&registered_node));
