...
fn validate_config(config: &HashMap<String, String>) -> Result<(), Errors> {
    if config.len() > 2 {
        return Err(Errors::ConfigError("Invalid len of arguments".to_string()));
    }
    ...
    match config.get(CQL_VERSION) {
        ...
        Some(_) => {
            return Err(Errors::ConfigError(format!(
                "CQL version must be {}, the version provided (CQL {}) is unsupported",
                VERSION_VALUE, config[CQL_VERSION]
            )))
        }
    ...
    if let Some(compression_value) = config.get(COMPRESSION) {
        if compression_value != COMPRESSION_VALUE {
            return Err(Errors::ConfigError(format!(
                "COMPRESSION must be {}, the value provided (COMPRESSION {}) is unsupported",
                COMPRESSION_VALUE, config[COMPRESSION]
            )));
        }
    }
    Ok(())
...
mod tests {
    ...
    fn test_validate_config_too_many_entries() {
        let bytes = vec![
            ...
            b'N', // "CQL_VERSION"
            0x00, 0x05, b'3', b'.', b'0', b'.', b'0', // "3.0.0"
            0x00, 0x0A, b'C', b'O', b'M', b'P', b'R', b'E', b'S', b'S', b'I',
            b'O', // "COMPRESSION"
            0x00, 0x03, b'i', b'z', b'4', // "iz4"
    ...
    fn test_validate_config_invalid_bytes_cql_version() {
        ...
        if let Err(Errors::ConfigError(msg)) = result {
            assert_eq!(
                msg,
                format!(
                    "CQL version must be {}, the version provided (CQL 2.0.0) is unsupported",
                    VERSION_VALUE
                )
            );
        } else {
