...
fn is_valid_bigint(input: &str) -> Option<Token> {
    if input.parse::<i64>().is_ok() {
        let literal = Literal::new(input.to_string(), DataType::Int);
...
mod tests {
    ...
    fn test_is_valid_text_correct_input() {
        ...
        let literal = Literal::new("valor1".to_string(), DataType::Text);
    ...
    fn test_is_valid_text_empty_input() {
        let input_vacio = "''";
        let result_vacio = is_valid_text(input_vacio);
        assert_eq!(result_vacio, None);
    }
    ...
    fn test_is_valid_bigint_positive() {
        ...
        let literal = Literal::new("12345".to_string(), DataType::Int);
        let token = Token::Term(Term::Literal(literal));
        assert_eq!(result, token);
    }
    ...
    fn test_is_valid_bigint_negative() {
        ...
        let literal_negativo = Literal::new("-98765".to_string(), DataType::Int);
